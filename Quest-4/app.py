...
def initialize():
    ...
    print(f"OPENAI_API_KEY detected is: {OPENAI_API_KEY}")

    if not ATLAS_URI:
        raise Exception ("'ATLAS_URI' is not set. Please set it in .env before continuing...")

    if not OPENAI_API_KEY:
        raise Exception ("'OPENAI_API_KEY' is not set. Please set it in .env before continuing...")
...
def run_vector_query (query, model_name):
    ...
    with output_container.container():
        ...
        movies = atlas_client.vector_search(collection_name = COLLECTION_NAME, 
...
if st.button("Recommend Movies"):
    ...
    #     'http://localhost:5000/vector_query',
    #     json={'query': user_query, 'model_name': selected_model}
    # )
